...
public class UIDrawCall : MonoBehaviour
{
	...
	void CreateMaterial ()
	{
		...
		{
            LogSystem.LogWarning(shaderName, " doesn't have a clipped shader version for ", mClipping);
	...
	void UpdateMaterials ()
	{
		...
		{
#if UNITY_EDITOR
            LogSystem.LogWarning("Hmm... This point got hit!");
#endif
	...
	public void UpdateGeometry ()
	{
		...
		if (count > 0 && (count == uvs.size && count == cols.size) && (count % 4) == 0)
		{
			...
			else
			{
                ...
                LogSystem.LogWarning("Too many vertices on one panel: ", verts.size);
			}
		...
		{
			if (mFilter.mesh != null) mFilter.mesh.Clear();
            LogSystem.LogWarning("UIWidgets must fill the buffer with 4 vertices per quad. Found ", count);
		}
	...
	void OnWillRenderObject ()
	{
		...
		if (mDynamicMat != null && isClipped && mClipping != Clipping.ConstrainButDontClip)
		{
            ...
            vTemp.x = -mClipRange.x / mClipRange.z;
            vTemp.y = -mClipRange.y / mClipRange.w;
            mDynamicMat.mainTextureOffset = vTemp;
            vTemp.x = 1f / mClipRange.z;
            vTemp.y = 1f / mClipRange.w;
            mDynamicMat.mainTextureScale = vTemp;

			Vector2 sharpness = Vector2.zero;
            sharpness.x = 1000.0f;
            sharpness.y = 1000.0f;
