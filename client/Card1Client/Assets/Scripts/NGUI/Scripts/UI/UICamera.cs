...
public class UICamera : MonoBehaviour
{
    ...
    private static PressureTouch[] s_curTouches = null;
    private static PressureTouch[] s_preTouches = null;

    /// <summary>
    /// 鑾峰彇touches涓?暟銆佁
    /// </summary>
    /// <value>The touches count.</value>
    ...
    void Awake()
    {
//         is3DTouchSupported = Check3DTouchFeatureSupported();
//         LogSystem.Log("is3DTouchSupported--->", is3DTouchSupported);
//         if (is3DTouchSupported)
//         {
//             s_curTouches = new PressureTouch[MAX_TOUCHES_COUNT];
//             for (int i = 0; i < MAX_TOUCHES_COUNT; i++)
//             {
//                 s_curTouches[i] = new PressureTouch();
//             }
//             s_preTouches = new PressureTouch[MAX_TOUCHES_COUNT];
//             for (int i = 0; i < MAX_TOUCHES_COUNT; i++)
//             {
//                 s_preTouches[i] = new PressureTouch();
//             }
//             RegistForceTouchCallback();
//         }
 ...
 || Application.platform == RuntimePlatform.BlackBerryPlayer
    ...
    void Start()
    {
        ...
        //JoyStickControlor.GetInstance().AddListener(OnStickInput);
    }
#else
    ...
    public void ProcessTouches()
    {
        ...
        if (is3DTouchSupported)
        {
            ...
            {
                Touch oritouch = Input.GetTouch(i);

                PressureTouch touch = s_curTouches[i];
                currentTouchID = allowMultiTouch ? touch.id : 1;
                ...
                currentTouch.delta = pressed ? Vector2.zero : touch.pos - currentTouch.pos;
                currentTouch.pos = touch.pos;
                ...
                // Double-tap support
                if (oritouch.tapCount > 1) currentTouch.clickTime = RealTime.time;
    ...
    public void ProcessTouch(bool pressed, bool unpressed)
    {
        ...
        if (pressed)
        {
            ...
            if (is3DTouchSupported)
            {
                ...
                {
                    if (currentTouch.enterDispatched == false)
                    {
                        Notify(currentTouch.pressed, "On3DPress", true);
                        currentTouch.enterDispatched = true;
                    }
                }
                else
                {
                    if (currentTouch.deltaPressure < 0f)
                    {
                        //鍘嬪姏鍦ㄥ噺灏忥紝璇存槑鍦ㄦ姮璧峨骞朵笖娲惧彂杩囪繘鍏ヤ簨浠殿
                        if (currentTouch.enterDispatched == true && currentTouch.exitDispatched == false)
                        {
                            Notify(currentTouch.pressed, "On3DPress", false);
                            currentTouch.exitDispatched = true;
                        }
                    }
                }
            }
    ...
    {
        s_preTouchesCount = s_curTouchesCount;
        s_curTouchesCount = 0;

        PressureTouch[] tempRef = s_preTouches;
        s_preTouches = s_curTouches;
        s_curTouches = tempRef;
    }

    private static int FindIndexOfTouchIdExist(int touchId, PressureTouch[] touches, int length)
    {
    ...
    /// <summary>
    /// 妫鏌をDTouch鍔熻兘鏄?惁琚?敮鎸併佁
    /// </summary>
    /// <returns><c>true</c>, if D touch feature supported was check3ed, <c>false</c> otherwise.</returns>
    public static bool Check3DTouchFeatureSupported()
    {
#if UNITY_IOS && !UNITY_EDITOR
		//return check_3dtouch_feature_supported ();
        return false;
#else
        return false;
#endif
    ...
    /// <summary>
    /// 鑾峰彇touches鏁扮粍銆佁
    /// 鏁扮粍闀垮害濮嬬粓鍥哄畾涓篗AX_TOUCHES_COUNT銆傚洜姝や笉瑕佷互Length涓轰笂闄愭潵閬嶅巻touches銆佁
    /// </summary>
    /// <returns>The touches.</returns>
    public static PressureTouch[] GetTouches()
    {
        if (is3DTouchSupported)
        {
            return s_curTouches;
    ...
    private delegate void OnForceTouchCallbackFromNativeDelegate(int touchId
         , float pressure, float maximumPossiblePressure, float pos_x, float pos_y, TouchPhase phase);
    private static void RegistForceTouchCallback()
    {
#if UNITY_IOS && !UNITY_EDITOR
        //regist_force_touch_callback(OnForceTouchCallbackFromNative);
#endif
    }

    // 鎺ユ敹Native灞傚彂閫佺殑Touch鏁版嵁
    [MonoPInvokeCallback(typeof(OnForceTouchCallbackFromNativeDelegate))]
    ...
    {
        int index = FindIndexOfTouchIdExist(touchId, s_curTouches, s_curTouchesCount);

        if (index == -1 && s_curTouchesCount < (int)MAX_TOUCHES_COUNT)
        {
            s_curTouches[s_curTouchesCount].id = touchId;
            s_curTouches[s_curTouchesCount].pos = new Vector2(pos_x * Screen.width, pos_y * Screen.height);
            s_curTouches[s_curTouchesCount].pressure = pressure;
            s_curTouches[s_curTouchesCount].maximumPossiblePressure = maximumPossiblePressure;
            s_curTouches[s_curTouchesCount].phase = phase;
            s_curTouches[s_curTouchesCount].eventTime = Time.time;

            index = s_curTouchesCount;
            ++s_curTouchesCount;
        }
        else
        {
            // Ended鍜孋ancelled鍙?浛鎹?egan,Moved鍜孲tationary 
            if ((phase == TouchPhase.Ended || phase == TouchPhase.Canceled)
               || (s_curTouches[index].phase != TouchPhase.Ended
                && s_curTouches[index].phase != TouchPhase.Canceled))
            {
                s_curTouches[index].id = touchId;
                s_curTouches[index].pos = new Vector2(pos_x * Screen.width, pos_y * Screen.height);
                s_curTouches[index].pressure = pressure;
                s_curTouches[index].maximumPossiblePressure = maximumPossiblePressure;
                s_curTouches[index].phase = phase;
                s_curTouches[s_curTouchesCount].eventTime = Time.time;
            }
        }

        // Add additioanl info.
        int preIndex = FindIndexOfTouchIdExist(touchId, s_preTouches, s_preTouchesCount);
        if (preIndex != -1)
        {
            s_curTouches[index].deltaTime = s_curTouches[index].eventTime - s_preTouches[preIndex].eventTime;
            s_curTouches[index].deltaPressure = s_curTouches[index].pressure - s_preTouches[preIndex].pressure;
        }
        else
        {
            s_curTouches[index].deltaTime = 0;
            s_curTouches[index].deltaPressure = 0f;
        }
    ...
    {
        JoyStickControlor stickControlor = JoyStickControlor.GetInstance();
        if (stickControlor.MoustStatus == JoyStickControlor.UIMouseStatus.MouseActive)
        {
#if UNITY_EDITOR || UNITY_STANDALONE_WIN
            useTouch = false;
            useMouse = false;
#endif
            //鍙?湁鍦ㄩ紶鏍囨縺娲荤姸鎬佹墠澶勭悊鐣岄潰浜嬩欢/
            //鐐瑰嚮纭??鎸夐敭锛屾淳鍙慜nClick浜嬩欢/
            if (stickControlor.GetKey(stickControlor.KeyCode_X) || stickControlor.GetKeyDown(stickControlor.KeyCode_X) || stickControlor.GetKeyUp(stickControlor.KeyCode_X))
            {
                ProcessStickMouse(code);
            }
        }
    }
    ...
    public void ProcessStickMouse(JoyStickControlor.StickButtonCode code)
    {
        // Update the position and delta
        JoyStickControlor stickControlor = JoyStickControlor.GetInstance();
        lastTouchPosition = stickControlor.MousePositon;
        mMouse[0].delta = lastTouchPosition - mMouse[0].pos;
        mMouse[0].pos = lastTouchPosition;
        bool posChanged = mMouse[0].delta.sqrMagnitude > 0.001f;
        ...
        if (stickControlor.GetKeyDown(stickControlor.KeyCode_X))
        {
            isPressed = true;
        }
        else if (stickControlor.GetKey(stickControlor.KeyCode_X))
        {
            isPressed = true;
        }
        ...
        bool pressed = stickControlor.GetKeyDown(stickControlor.KeyCode_X);
        bool unpressed = stickControlor.GetKeyUp(stickControlor.KeyCode_X);
	...
	//private extern static bool check_3dtouch_feature_supported ();

	//[DllImport("__Internal")]
	//private extern static void regist_force_touch_callback (OnForceTouchCallbackFromNativeDelegate native_callback);
#endif
}
