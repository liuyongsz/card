...
public class NGUIEditorTools
{
    ...
    static public void DrawSprite (Texture2D tex, Rect rect, UISpriteData sprite, Color color, float scaleX, float scaleY)
	{
		DrawSprite(tex, rect, sprite, color, null, scaleX, scaleY);
	}
    ...
    static public void DrawSprite (Texture2D tex, Rect drawRect, UISpriteData sprite, Color color, Material mat, float scaleX, float scaleY)
	{
		...
		{
			Rect uv = new Rect(sprite.x, sprite.y, sprite.width, sprite.height);
			uv = NGUIMath.ConvertToTexCoords(uv, (int)(tex.width * scaleX), (int)(tex.height * scaleY));
			GUI.DrawTextureWithTexCoords(outerRect, tex, uv, true);
		}
		else
		{
