...
public class ShareDemo : MonoBehaviour {
    ...
    void shareWeChat()
    {
        ...
        //shareSdk.ShowPlatformList(null, content, 100, 100);                      //弹出分享菜单选择列表  
        ssdk.ShowShareContentEditor(PlatformType.WeChat, content);                 //指定平台直接分享  
...
//			content.SetMusicUrl("http://mp3.mwap8.com/destdir/Music/2009/20090601/ZuiXuanMinZuFeng20090601119.mp3");
        ...
        btnWidth += 80 * scale;
        btnTop += btnHeight + 20 * scale;
        if (GUI.Button(new Rect((Screen.width - btnWidth) / 2, btnTop, btnWidth, btnHeight), "分享QQ"))
        {
            share();
        }
        ...
        btnWidth += 80 * scale;
        btnTop += btnHeight + 20 * scale;
        if (GUI.Button(new Rect((Screen.width - btnWidth) / 2, btnTop, btnWidth, btnHeight), "分享WeChat"))
        {
            shareWeChat();
        }
    }
